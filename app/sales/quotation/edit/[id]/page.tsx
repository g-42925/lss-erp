...
export default function EditQuotation({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <EditQuotationContent params={params} />
    </Suspense>
  )
}

function EditQuotationContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const quoRes = await fetch(`/api/web/quotations/${id}`)
      ...
      if (!quoData.error && quoData.result) {
        ...
        if (quo.priceOptions && quo.priceOptions.length > 0) {
          setPriceOptions(quo.priceOptions)
        }
        if (quo.programs && quo.programs.length > 0) {
          setPrograms(quo.programs)
        }
        if (quo.note) setNote(quo.note)
        if (quo.introduction) setIntroduction(quo.introduction)
        if (quo.disclaimers && quo.disclaimers.length > 0) {
          setDisclaimers(quo.disclaimers)
        }
        if (quo.status) setStatus(quo.status)
      }
    } catch (e) {
  ...
  if (!hasHydrated || initLoading) return <div className="p-8 text-center"><span className="loading loading-spinner loading-lg"></span></div>
    ...
    <div className="p-8 max-w-4xl mx-auto pb-32">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg space-y-6">
        ...
        <div className="form-control w-full border-b pb-6">
          ...
          <div className="flex flex-row gap-3 items-center">
            <label className="label"><span className="label-text">Quotation Status</span></label>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={loading} className="btn btn-success text-white">
            {loading ? <span className="loading loading-spinner"></span> : 'Update Quotation'}
          </button>
        </div>
      </form>
    </div>
