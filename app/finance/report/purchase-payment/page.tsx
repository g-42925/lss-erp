...
type PaymentRow = {
  ...
  supplierData?: { bussinessName?: string, name?: string }
  vendorData?: { name?: string, bussinessName?: string }
  method: string
  date: string
  amount: number
}
...
export default function PurchasePaymentReportPage() {
  ...
  useEffect(() => {
    if (hasHydrated && masterAccountId && !hasRun) {
  ...
  function runReport(sd: string, ed: string) {
    ...
    setLoading(true)
    setReport([])
    fetch(`/api/web/finance/reports/purchase-payment?id=${masterAccountId}&startDate=${sd}&endDate=${ed}`)
      .then(r => r.json())
      .then(data => {
        if (!data.error) {
  ...
  const filtered = search.trim()
    ? report.filter(r => {
        const isProcurement = r.purchaseType === 'procurement';
        const counterpartName = isProcurement ? "Procurement" : (r.supplierData?.bussinessName || r.supplierData?.name || r.vendorData?.bussinessName || r.vendorData?.name || "");
        return r.purchaseOrderNumber?.toLowerCase().includes(search.toLowerCase()) ||
               r.paymentNumber?.toLowerCase().includes(search.toLowerCase()) ||
               counterpartName.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <>
        ...
        {hasRun && !loading && (
          <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-red-100 bg-red-50 p-5 shadow-sm">
              <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-red-600">Total Nominal Keluar</p>
              <p className="text-3xl font-bold text-red-700">Rp {totalAmount.toLocaleString("id-ID")}</p>
            </div>
          </div>
        )}
        ...
        <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
          {hasRun && !loading && (
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-3">
              ...
              <input
                type="search"
                placeholder="Cari PO, vendor, atau metode…"
                value={search}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wider text-slate-500">
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left">No referensi</th>
                    <th className="px-5 py-3 text-left">Supplier/Vendor</th>
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-3.5 font-medium text-slate-800">
                          {row.paymentNumber}
                          <span className="block text-xs font-normal text-slate-400 mt-0.5">{row.purchaseOrderNumber}</span>
                        </td>
                        <td className="px-5 py-3.5 text-slate-600">
                          {counterpartName}
                          {isProcurement && <span className="ml-2 rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700">PROCUREMENT</span>}
                        </td>
                        <td className="px-5 py-3.5 text-slate-600">
                ...
                <tfoot className="border-t-2 border-slate-200 bg-slate-50 text-xs font-bold uppercase text-slate-500">
                  <tr>
                    ...
                    <td className="px-5 py-3 text-right text-red-700">Rp {totalAmount.toLocaleString("id-ID")}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
      ...
      <div className="hidden print:block text-black bg-white absolute top-0 left-0 w-full min-h-screen z-50 p-8">
        <div className="text-center mb-6 border-b-2 border-black pb-4">
          <p className="text-2xl font-bold uppercase tracking-widest">Laporan Pengeluaran Pembelian</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            ...
            { label: "Total Nominal", value: `Rp ${totalAmount.toLocaleString("id-ID")}`, color: "text-red-800" },
          ].map(c => (
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="border-b-2 border-black font-bold text-left bg-gray-50">
              ...
              <th className="py-2 px-2">No Referensi</th>
              <th className="py-2 px-2">Supplier/Vendor</th>
              ...
              return (
                <tr key={idx} className={`border-b border-gray-200 ${idx % 2 === 1 ? "bg-gray-50" : ""}`}>
                  ...
                  <td className="py-1.5 px-2 font-semibold">
                    {row.paymentNumber}
                    <span className="block text-[10px] font-normal text-gray-500">{row.purchaseOrderNumber}</span>
                  </td>
                  <td className="py-1.5 px-2 text-gray-600">{counterpartName}</td>
                  <td className="py-1.5 px-2 text-center">{row.method || "—"}</td>
                  <td className="py-1.5 px-2 text-right font-bold text-red-900">Rp {row.amount.toLocaleString("id-ID")}</td>
                </tr>
              )
            })}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-black font-bold bg-gray-100">
              ...
              <td className="py-2 px-2 text-right text-red-900">Rp {totalAmount.toLocaleString("id-ID")}</td>
            </tr>
          </tfoot>
        </table>
